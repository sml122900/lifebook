...
// 이용약관 — 공정거래위원회 전자상거래(인터넷사이버몰) 표준약관
// 제10023호(2015.6.26 개정) 전문(제1조~제24조) 축조 그대로 게시. 조항의
// 임의 창작·요약·삭제 없음. "OO 회사"/"OO 사이버 몰" 빈칸만
// BUSINESS_INFO.companyName 으로 채움 — 그 외 본문은 표준약관 원문 그대로.
// /privacy 와 레이아웃·스타일 통일.

export const metadata = {
  title: "이용약관 — 라이프북",
  description: "라이프북 서비스 이용약관.",
};

const MALL = BUSINESS_INFO.companyName;
...
const ARTICLES: ArticleData[] = [
  {
    title: "제1조 (목적)",
    blocks: [
      `이 약관은 ${MALL}(전자상거래 사업자)가 운영하는 ${MALL}(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다.`,
  ...
  },
  {
    title: "제2조 (정의)",
    blocks: [
      `① "몰"이란 ${MALL}가 재화 또는 용역(이하 "재화 등"이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.`,
  ...
  },
  {
    ...
    blocks: [
      `① "몰"은 이 약관의 내용과 상호 및 대표자 성명, 영업소 소재지 주소(소비자의 불만을 처리할 수 있는 곳의 주소를 포함), 전화번호·모사전송번호·전자우편주소, 사업자등록번호, 통신판매업 신고번호, 개인정보관리책임자등을 이용자가 쉽게 알 수 있도록 ${MALL}의 초기 서비스화면(전면)에 게시합니다. 다만, 약관의 내용은 이용자가 연결화면을 통하여 볼 수 있도록 할 수 있습니다.`,
...
function ArticleSection({ title, blocks }: ArticleData) {
  return (
...
export default function TermsPage() {
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-12">
      ...
      <h1 className="mt-4 text-ink">이용약관</h1>

      <p className="mt-4 text-base text-ink-faint">
        이 약관은 공정거래위원회 전자상거래(인터넷사이버몰) 표준약관
        제10023호(2015.6.26 개정)를 기준으로 작성되었습니다.
      </p>

      <div className="mt-10 flex flex-col gap-8">
        {ARTICLES.map((a) => (
          <ArticleSection key={a.title} {...a} />
        ))}

        <section className="flex flex-col gap-2">
          <h2 className="text-ink">부칙</h2>
          <p className={P}>이 약관은 2026년 9월 11일부터 시행합니다.</p>
        </section>
      </div>
      ...
      <section className="mt-12 border-t border-line pt-6 text-base text-ink-faint">
        <dl className="flex flex-col gap-1">
          ...
          <Row label="사업자등록번호" value={BUSINESS_INFO.bizRegNo} />
          <Row label="주소" value={BUSINESS_INFO.address} />
          <Row
            label="문의"
            value={`${BUSINESS_INFO.csEmail} · ${BUSINESS_INFO.csPhone}`}
          />
        </dl>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-wrap gap-x-2">
      ...
      <dd className="text-ink-faint">{value}</dd>
    </div>
  );
}
