...
export default async function RoomDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-8 px-6 py-10">
      ...
      <section>
        <h2 className="text-xl font-bold text-ink">멤버</h2>
        <ul className="mt-3 flex flex-col divide-y-2 divide-zinc-200 overflow-hidden rounded-md border-2 border-line bg-surface">
          {room.members.map((m) => (
            <li
              key={m.id}
      ...
      <section className="rounded-md border-2 border-line bg-surface p-6">
        <h2 className="text-xl font-bold text-ink">초대하기</h2>
        <p className="mt-2 text-base text-ink-soft">
        ...
        {room.invites.length > 0 && (
          <ul className="mt-5 flex flex-col gap-3">
            {room.invites.map((inv) => (
              <li
                key={inv.id}
                className="rounded-md border-2 border-line bg-canvas px-4 py-3"
              >
                <p className="text-base text-ink-soft">초대 링크</p>
                <p className="mt-1 break-all text-base font-mono text-ink">
      ...
      <section className="flex flex-col gap-4">
        <h2 className="text-2xl font-bold text-ink">함께 보는 타임라인</h2>
        <TimelineLegend />

        {sortedYears.length === 0 ? (
          <div className="rounded-md border-2 border-dashed border-line bg-canvas p-6">
          ...
          <ol className="flex flex-col gap-12">
            {sortedYears.map((year) => {
              ...
              return (
                <li key={year}>
                  <h3 className="mb-5 text-3xl font-bold text-ink">
                    {year}
                  </h3>
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-2xl font-bold text-ink">공동 추억 추가</h2>
