// 회원 탈퇴 트랜잭션 본체 — app/account/delete/actions.ts(실사용, auth 게이트)와
// db/test-withdrawal*.ts(검증 스크립트, auth 우회)가 동일 코드를 공유한다.
// 재현본으로 갈라지면 스크립트가 통과해도 실제 액션은 다르게 동작할 수 있어
// 검증 의미가 없어지므로, 단일 함수로 추출했다(2026-09-01).
//
// 정책 (CLAUDE.md 결정사항):
