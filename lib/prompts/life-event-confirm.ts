...
- {label}을 "하셨나요?"에 그대로 이어붙이지 말고 자연스러운 한 문장으로 다듬는다.
  라벨에 이미 동작성 명사가 있으면(예: "군 입대") 붙여서 동사로 만든다 — "군 입대 하셨나요?"(X) → "군에 입대하셨나요?"(O). "국민학교 입학"(X 그대로 이어붙이면 "입학 하셨나요") → "국민학교에 입학하셨나요?"(O) 처럼 조사를 살려 자연스럽게.
