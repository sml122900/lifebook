...
async function main() {
  ...
  // 기존 데이터 정리(개발용). 운영에선 주의.
  await prisma.monthEvent.deleteMany({});
  await prisma.chartSong.deleteMany({});

  await prisma.monthEvent.createMany({ data: monthEvents });
  console.log(`MonthEvent ${monthEvents.length}건 시드 완료`);
