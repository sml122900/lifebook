...
7. **버튼·링크로 페이지 이동이 새로 생기거나 바뀌면 `docs/navigation-map.md` 를 같은 커밋에서 갱신한다.** 대상: 새 `<Link href>`·`router.push`/`replace`·`redirect`·`signIn/signOut redirectTo`·form action 페이지 전환·외부 링크(검색·지도·결제·mailto). 라우트를 추가·삭제·리다이렉트로 바꿀 때, 사이드 패널/헤더/위젯 등 전역 메뉴를 바꿀 때도 해당 표에 반영. 표기는 문서 상단 범례(`→ /경로`·`↗ https://`·`⚙ 액션`)를 따른다.
...
| **장소 1:N (H1~H5)** | **한 사건에 여러 장소 — 새 모델 `MemoryPlace`(UserMemory 1:N, sortOrder) + 백필 멱등 마이그 2건. 5컬럼 호환 유지(H6 제거 예정). 읽기(places[] 3함수+사진+인물)·쓰기(normalizePlaces·중첩 create·update=$transaction 삭제+재생성)·표시(PlacePreview 배열·세로 칩)·다중 입력(새 `PlacesEditor`=PlaceSearchInput 재사용, EventForm·CategoryForm·PhotoPlaceModal). updatePhotoMemoryPlaces 소유 가드 보안 픽스** | (`2026-06-25` 일지) | ✅ H1~H5 완료 (H6 후속) |
| **추출 품질 픽스 (P0+S1~S6)** | **외할머니 인터뷰 9개 실측(P0) → 7결함 단계 픽스. S1 프라이버시 리댁션 전처리(`lib/transcript-redact.ts`, 결정적 윈도우, 추출 입력만·원본 보존) / S2 중복회피(사건=프롬프트+코드 dedup 조사처리, 인물=코드 dedup) / S3 자식 마일스톤 가드(주어=어르신) / S4 긴 전사 청크(splitOneChunk·overlap·maxTokens 4096, 0618-2 0→7건) / S5 birthYear 파생(resolveBirthYear) / S6 취향 분리(preferences[])+이름 단정 완화. 3런 합본 검증 통과, 정답지 9개 전부 커버** | (`2026-06-25` 세션2 일지) | ✅ 완료 (soft 잔존 수용) |
| **포스터 파이프라인 (P1~P7-b)** | **연혁→인생 나무 포스터 풀스택. P3 문장(refineForPoster·Sonnet)·P1 선별(node/memo·무스키마)·P2 선택 UI+`Poster` 모델·P4 SVG 합성(`compose-layout` 순수+`PosterCompose` 클라 getBBox/luminance·river-bg.png)·`/poster` 템플릿 선택(강물 실작동·나머지 준비중)·P6 액티브 편집(per-item override `lib/poster/overrides`·드래그·크기·내용·빼기·안전여백)·P7-a 인쇄 export(Canvas 2D 5008×7063 300dpi PNG·pHYs 메타)·P7-b 주문(ProductOrder 재질 39k/99k·Toss 재사용·실결제 flag OFF)+발주(`/admin/orders` ADMIN_EMAILS·posterSnapshot)+환불(발주 전). `OrderFulfiller` 어댑터** | (`2026-06-25` 세션3 일지) | ✅ 완료 (assistant 칩·PDF·이메일발주 후속) |
| **AI 모델 선택 (전역 라이브)** | **사용자가 라이브 응답 모델(haiku/sonnet/opus) 전역 선택. `User.aiModel`(기본 haiku)+`lib/ai-model`·`lib/user-ai-model`(단일 소스). 라이브(companion 신규차감·refine·assistant·tutorial 클램프)=전역 모델+배율 {1,3,8}(opus 5→8 통일). 추출=Sonnet 고정(opus 차단). `AiModelChips`(설정·companion) 전역 갱신, 기존 토글(refine tier·assistant depth·온보딩 model) 통합 제거** | (`2026-06-25` 세션4 일지) | ✅ 완료 (companion 유료·tutorial 무료) |
| **코치마크 온보딩** | **첫 사용자 가이드 오버레이 — 공용 `CoachMarks` 엔진(box-shadow 구멍 spotlight·rAF 폴링+ResizeObserver 측정·단계별 패널 제어·미존재 타겟 skip) + `lib/tours.ts` steps + 화면 `data-tour`. 메인 5단계 → 포스터 3화면(`ScreenTour` 래퍼·도움말 버튼, 엔진 0줄 확장). `User.completedTours String[]` 1컬럼(첫 1회 자동·완료 멱등 raw SQL·재실행 START_TOUR_EVENT)** | `docs/decisions/coachmark-onboarding.md` · (`2026-06-27` 일지) | ✅ 완료 |
| **샘플 포스터 + 배경 다양성/픽스** | **랜딩 샘플 3종(김순자·박성호·이지은) + 익명화 프리미엄(이정순, river 재합성·이유옥 PII 0)을 실제 파이프라인으로 export. ★ export 배경 하드코딩 버그 수정(맞춤배경이 인쇄본에 안 들어가던 실고객 영향) + 맞춤배경 색조 다양성(hue=취향/채도=낮게)·검수 hue-agnostic(채도 78→120·강 검출 물빛+대비)·이유옥 성공작 6요소 재강화** | (`2026-06-30` 일지) | ✅ 완료 |
| **랜딩 v2 리뉴얼** | **"말로 이야기→AI→포스터" 3초 첫인상 — 히어로(프리미엄 포스터)·작동3단계(녹음 강조)·결과물 갤러리(4색조)·제품(포스터 메인·책/씨앗 준비중)·안심·CTA. 샘플 webp 4장 커밋(untracked 404 픽스)** | (`2026-06-30` 일지) | ✅ 완료 |
| **기능1 — 노드 연도 기본 숨김** | **어르신 연도 부정확 대비 — `Poster.showNodeYears @default(false)`(마이그). 정렬은 연도 기준 유지·표시만 분기. 편집 전체 토글 + P4/export/주문 스냅샷/관리자 발주 반영. 연도 숨김 시 박스 측정서 연도줄 제외→제목 중앙** | (`2026-06-30` 일지) | ✅ 완료 |
...
| **v3 통합 채팅(`chat-v3`) — LifeEvent/Episode/OnboardingProfile** | **`/chat-v3`(생년→학령·병역 등 골격 확인질문+심화 에피소드 대화) 신규 모델 2종 도입 — 이 표에 세션별 항목이 없어 상세 불명(전용 phase 문서 없음, `docs/daily`에도 미기록). 관련 코드: `app/chat-v3/`·`app/actions/{onboarding,onboarding-v3,episode,chat-v3-log,life-event,gaps,open-chat}.ts`·`lib/episode.ts`. 커밋 로그 `v3 P1~P4`·코드 주석 `STAGE1~4` 참조** | (커밋 로그만, 일지 없음) | ⚠ 소급 기록 — 상세 후속 필요 |
| **탈퇴 FK cascade 픽스 + v3 채팅 UX 마무리** | **탈퇴 시 `LifeEvent_userId_fkey` 위반으로 실패하던 버그 — `LifeEvent`/`OnboardingProfile`(User 관계)·`Episode`(LifeEvent 관계)에 `onDelete:Cascade` 추가(마이그 `20260901010000`, 순수 FK 강화). 탈퇴 트랜잭션을 `lib/account-deletion.ts`(`deleteAccountTx`)로 추출해 `app/account/delete/actions.ts`(실사용)와 `db/test-withdrawal.ts`(검증)가 동일 코드 공유(재현본 드리프트 방지) + 시나리오4 신규(Episode 는 실제 `createEpisodeBridge()` 호출) 검증 통과, 고아 행 0건. 곁들여 P5-4(캐릭터 헤더 sticky)·P5-5(확인질문 `withJosa` 자연화)** | `docs/daily/2026-09-01.md` | ✅ 완료 |
...
- [x] 장소 1:N(H1~H5, 2026-06-25) → 한 사건(UserMemory)에 여러 장소. 새 모델 `MemoryPlace`(memoryId FK Cascade·sortOrder) additive only. **기존 평면 5컬럼(placeName/placeAddress/lat/lng/placeSource)은 호환·롤백용으로 H5까지 유지**(H6에서 DROP). 백필 멱등(`NOT EXISTS` 가드·`gen_random_uuid()`). 읽기=3함수 `places[]` include(sortOrder)·사진·인물. 쓰기=`normalizePlaces`(places[] 우선·단일 place 흡수·validatePlace) + create 중첩 + update `$transaction([update,deleteMany,createMany])` 원자적. 표시=`PlacePreview` 배열 세로 칩(빈 배열 렌더X). 입력=새 `PlacesEditor`(PlaceSearchInput 재사용, 추가/제거)·EventForm·CategoryForm·PhotoPlaceModal. ⚠️ `updatePhotoMemoryPlaces` 트랜잭션화 시 MemoryPlace엔 userId 없어 소유 가드를 트랜잭션 *앞*에(남의 장소 삭제 차단). 단일 place 호환 분기·5컬럼·룸/인물 미표시는 H6 후속
- [x] 추출 품질 픽스(P0+S1~S6, 2026-06-25 세션2) → 외할머니 인터뷰 9개 실측 후 7결함 단계 픽스. **다층 방어 전제**(리댁션·dedup=1차 거름, 뒤에 draft 검토·주문전 확인·연혁 삭제) → 과삭제·오스킵보다 "좋은 내용 살리기" 우선. S1 프라이버시=**결정적 윈도우 리댁션 전처리**(프롬프트 차단은 확률적이라 불충분, 추출 입력에만·원본 보존). S2 인물 dedup=**코드 레벨**(프롬프트 주입은 친척 목록 보여주면 새 인물까지 과억제 실측). S2 사건 dedup=프롬프트(1차)+코드(2차, 보수적 토큰·조사처리). S3 자식 가드=주어가 어르신일 때만 사건(자식 독립사건은 인물로). S5 birthYear=컬럼→BIRTH 이벤트 파생 fallback. S6 취향=`SplitResult.preferences[]` 분리(포스터용, 저장 후속). 수용한 soft 잔존: 트리거서 먼 거부 서술(어머니 memo "의사 혼처 선호")은 다층 방어로 흡수(100% 누출 0 강박 아님). 검증 스크립트 `scripts/extract-test.ts`(읽기 전용·gitignore)
- [x] 포스터 주문/발주/환불(P7-b, 2026-06-25 세션3) → 기존 `ProductOrder`/Toss/`settleProductOrder` **재사용**(결제 보안 재구현 0). 재질=`optionId`(일반39k/프리미엄99k, products.ts 단일 출처). 실결제 `POSTER_PAYMENT_LIVE_ENABLED` 기본 OFF(STT 패턴). 발주=관리자 페이지 `/admin/orders`(ADMIN_EMAILS 화이트리스트 게이트 proxy+layout+액션 3중) — **주문 시점 `posterSnapshot`**(refine 결과 freeze)으로 P7-a export(이메일 provider 없어 페이지 방식). `OrderFulfiller` 인터페이스(ManualFulfiller no-op → 추후 이메일/API 교체). 환불=발주 전(`paid`)만, 사용자 요청 → 관리자 `refunded`. P7-a=Canvas 2D 재렌더(SVG 직렬화·CJK 임베드 회피)·pHYs 300dpi 메타. 마이그 `20260625100000`
- [x] AI 모델 선택(전역 라이브, 2026-06-25 세션4) → `User.aiModel`(haiku/sonnet/opus, 기본 haiku). 단일 소스 `lib/ai-model`(상수)·`lib/user-ai-model`(get/set). **라이브 응답=전역 모델**(companion·refine·assistant·tutorial), **추출=Sonnet 고정**(opus 차단, getUserAiModel 참조 0). 배율 **{1,3,8} 통일**(MODEL_MULTIPLIER opus 5→8, REFINE는 별칭). 차감 정책(경영방+사용자 확정): **companion 유료(신규 ×배율)·tutorial 무료(opus→sonnet 클램프)**·refine·assistant ×배율. `AiModelChips`(full=설정/compact=companion)→`updateAiModel` 전역 갱신(세션한정 아님, 화면서 바꾸면 일관). 기존 토글(refine tier·assistant depth·온보딩 model) 통합 제거. assistant 칩은 부모 5곳 threading 회피로 안내문(후속). 마이그 `20260625110000`
- [x] 코치마크 온보딩(2026-06-27) → 공용 `CoachMarks` 엔진 1개 + `lib/tours.ts` steps + 화면 `data-tour` 속성. spotlight=거대 box-shadow 구멍(타겟 z-index 무관, 사이드 패널 위로도). 측정=rAF 폴링+안정 프레임 감지+`ResizeObserver(body)`+`scrollIntoView({behavior:"instant"})`(트랜지션·비동기 렌더·layout shift 흡수, 단발 측정 어긋남 해소). 말풍선=타겟 위/아래 여유 공간 계산, 둘 다 부족하면 화면 위/아래 고정. 저장=`User.completedTours String[]` 1컬럼(마이그 `20260626140000`, 멱등 `array_append(array_remove(...))`), 재실행=`START_TOUR_EVENT`(완료 사용자도 항상 마운트·RSC 의존 0). 메인 5단계 + 포스터 3화면(`ScreenTour` 래퍼·좌하단 도움말 버튼)을 엔진 0줄로 확장. `docs/decisions/coachmark-onboarding.md`
- [x] export 배경 = 표시 배경(2026-06-30) → `print-export.ts` 가 river-bg 하드코딩이라 맞춤배경이 인쇄/주문 PNG 에 안 들어가던 실고객 버그. `exportPosterPng(bgSrc)` 인자 + `PosterCompose` 가 화면과 동일 `bgSrc` 전달(custom=`/api/poster/background`, 캐시 무력화 쿼리·same-origin). `7863309`
- [x] 맞춤배경 다양성 정책(2026-06-30) → **색상(hue)=취향대로 / 채도=낮게**(은은함 속 다양성). 검수는 hue-agnostic — 채도 상한 78→120(`max-min` 이 따뜻색 과대평가 → 과채도만 차단), 강 검출은 청록 전제 폐기하고 "중앙 띠 물빛 + 강가 대비". 이유옥 성공작 6요소(펜&잉크·샘→바다·#FAF7F0·잔잔감성·음악 은유·꽃 드문드문) 복원, 다양성은 [취향색] 강물·색기운 + [취향꽃]으로. `446836e`·`bd84748`
- [x] 노드 연도 기본 숨김(2026-06-30, 기능1) → `Poster.showNodeYears @default(false)`. **정렬은 항상 연도 기준·표시만 분기.** 편집 전체 토글(개별 X) + P4/export/주문 스냅샷/관리자 발주 반영. 연도 숨김 시 박스 측정서 연도줄 제외→제목 박스 중앙. `434ad3c`
- [x] 시대 대사건(2026-06-30, 기능2) → 노드 개인 연도 숨긴 대신 **객관적 역사 대사건을 노드 사이 시간 닻으로**. `era-events.ts` 26건 3티어(순수)·`getEraEventsForPoster`(생애범위[출생 이후]·티어 0/1/2/3·removedIds). `placeEraEvents`=연도→노드 y 보간·강 중앙 검정(메모=회색 구분)·**노드 bbox `[top-40,bottom+40]` 회피**(연도 ON 박스 포함)·과밀은 균등분산(대사건=강중앙·노드=±200 오프셋이라 가로 겹침 0). 설정=`eraTier`/`removedEraEvents`(마이그), 클라 즉시 필터+서버 영속+스냅샷 freeze. 강건화: 퍼즈(NaN·Infinity·역전bbox·과밀) "안 터지고 유한". `8c59cd0`~`c494e12`
- [x] 말동무 코치마크(2026-06-30) → `CoachMarks`·`ScreenTour` 엔진 무수정 + `COMPANION_TOUR_STEPS` 4단계(채팅·🎤 녹음강조·스토리 패널·대화 마치기) + `data-tour`. ★ 입력창이 오프닝(API) 후 렌더 → page autoStart 대신 `CompanionClient` 가 phase=idle 직후 `START_TOUR_EVENT` 로 첫 방문 1회 자동(모든 타겟 존재 후 시작). 동적 요소(EraArchive·메시지) 타겟 제외. `9cf0816`
...
- [x] 탈퇴 트랜잭션 단일 소스(2026-09-01) → 실사용 액션과 검증 스크립트가 각자 트랜잭션을 복붙해 갈라져 있으면 스크립트 통과가 실제 동작을 보장 못 한다는 지적 → `lib/account-deletion.ts`(`deleteAccountTx`) 로 추출해 `app/account/delete/actions.ts`·`db/test-withdrawal.ts` 양쪽이 동일 함수 호출. 신규 `LifeEvent`/`OnboardingProfile`/`Episode` 는 User/LifeEvent 관계에 `onDelete:Cascade` 필수(마이그 `20260901010000`) — 새 모델 추가 시 다른 모델과 동일 패턴 유지할 것.
