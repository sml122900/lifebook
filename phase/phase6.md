...
## 추후 튜닝 메모 (Phase 6 이후 시드 확장 시 재점검)

6.6 검증에서 두 페르소나(1965년생/1995년생) 모두 상위 10곡이 **회상 절정 구간(13~25세)에 몰려 bump=1.00을 독식**하는 현상을 확인. 시드가 69곡 규모일 때는 정상 동작이지만, 시드 확장 시 다음 두 가지를 재시험한다.

1. **bump=1.00 독식 — 가중치 균형 재시험**
   회상 절정 구간 밖이지만 취향이 더 강하게 매칭되는 곡(예: 사용자가 `favMusic`에 적은 곡)이 가중치 때문에 밀려나지 않는지 확인. 현재 weight 단계(`1.0 / 0.7 / 0.4`)가 너무 가파를 수 있음 → smoother 함수(가우시안 등) 검토. 진단 컬럼(`bump_weight`, `distance`, `score`) 출력은 계속 유지해서 튜닝 근거 확보.

2. **distance 변별력 약함 (0.50~0.68 좁은 구간)**
   상위 10곡의 cosine distance가 0.18 폭 안에 모여 추천이 "그 시대 아무 곡" 느낌으로 흐를 위험. 임베딩 텍스트(`embeddingTextFor`)에 **장르/분위기 태그**(예: "발라드", "댄스", "트로트", "록", "잔잔한", "신나는")를 추가해 변별력 보강. `db/seed/musicEvents.ts`의 `description` 필드를 활용하거나 별도 `tags` 필드 도입 검토.
