...
**작업**
- `lib/musicbrainz.ts`: MusicBrainz API로 곡/아티스트 검색 → 정규 발매연도·MBID·아티스트 보강.
- ⚠️ **에티켓 준수**: 식별 가능한 `User-Agent` 헤더 필수, **요청 1초당 1회** 레이트리밋.
- 시드 항목을 MBID로 매칭해 메타데이터 채우고, 필요 시 카탈로그 확장.

**완료 기준**: 시드 곡들이 MusicBrainz 메타데이터로 보강되며, 레이트리밋·User-Agent를 지킨다.
