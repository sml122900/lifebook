...
> 작성: 2026-06-26 · 최신화: 2026-09-12(PG 심사 대응 — `/terms` 이용약관 페이지 신설 + 푸터·consent 링크 연결) · 코드 기준 정리.
...
사업자 정보(상호·대표자·사업자번호·주소·CS·통신판매업)는 텍스트 표시.
| 버튼 | 이동 |
|---|---|
| 환불정책 | → `/refund` |
| 이용약관 | → `/terms` |
...
| `/consent` | 비로그인 → `/login` · 동의완료 → `/enter` · "개인정보 처리방침 자세히 보기" → `/privacy`(새 창) · "이용약관 자세히 보기" → `/terms`(새 창) · 시작하기 → `/enter` |
