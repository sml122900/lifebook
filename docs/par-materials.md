...
## 설문형 온보딩을 대화형으로 전환 — LLM 파싱·위젯·인물 추출 3단 경로 설계

- **Problem**: 기존 온보딩 폼이 11개 필드를 한 번에 보여줘 이탈률 우려. 어르신 타깃 서비스라 텍스트 입력 부담이 크고, "거주지를 장소로 매핑", "언급된 가족을 인물록에 추가" 등 추가 데이터 수집이 필요했음.
- **Action**: 채팅 UI(Next.js Server Action + 클라 상태 머신)로 11질문을 한 번에 하나씩 표시. 입력 경로를 3가지로 분리 — ① 위젯(칩/연도/항목 리스트)은 LLM 파싱 0(직접 저장), ② 자유 텍스트는 Haiku 파싱, ③ 장소 답변엔 `PlaceSearchInput`(기존 컴포넌트 재사용) 인라인 토글로 좌표 추가. 온보딩 완료 직전 siblings·parentsInfo·closeFriends 텍스트를 Sonnet에 전달해 인물 후보 추출 → 채팅으로 하나씩 성함 확인 → `createPerson(isDraft=false)` 즉시 확정(AI 추론 draft 경로와 분리).
- **Result**: 4단계(B1·B2·D·E) 4커밋으로 채팅 온보딩 완성. 위젯 경로는 LLM 비용 0, 텍스트 경로는 Haiku 저비용 파싱으로 이중 경로가 비용 최적화. 온보딩 답변이 `lib/companion.ts` `buildSystemPrompt` 에 자동 주입돼 음성 동반자가 "음악 좋아하셨다고 하셨는데…" 개인화 활용 가능. *교훈*: "위젯으로 선택 가능한 것은 LLM을 거치지 않는다"는 원칙이 UX와 비용 두 축을 동시에 개선.
